...
public class VehicelWise_Deatail extends Activity implements View.OnClickListener, OnMapReadyCallback,
    ...
    MyTextView etp_title, source, destination, dates ;
    TextView start_day,ign, code_title,gps,valid_date, circles, credit, dir_text, pick_time, pick_date, min_name, quitity, vehi_num, devi_id, tranpo_name,current_date_time;
...
//TODO : ADD ARIAL DISTANCE IN SEARCH DETAIL
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_vehicel_wise__deatail);
                    ...
                    try {
                        ...
                        if(jsonArray.length() > 0) {
                           ...
                           // ign.setText(jsonObject.getString("ignnumber"));
                            ...
                            //  double nera_by_me =    distance(Double.parseDouble(jsonObject.getString("latitude")),Double.parseDouble(jsonObject.getString("longitude")), Double.parseDouble ( lat ),Double.parseDouble ( lag ));
                           // vehicle_near_by_truck.setText ( nera_by_me +"");
                          //  Toast.makeText ( VehicelWise_Deatail.this , ""+nera_by_me , Toast.LENGTH_SHORT ).show ( );
                            ...
                            try {
                                ...
                                SimpleDateFormat df = new SimpleDateFormat("dd-MM-yyyy");
                                String totalinstalled = jsonObject.optString("datereceived");

                               // printDifference()
                                String[] sdata = totalinstalled.split("-");
                                int spacePos = sdata[2].indexOf(" ");
                                if (spacePos > 0) {
                                    sdata[2] = sdata[2].substring(0, spacePos );
                                }
                              //  start_day.setText(sdata[2]+"-"+sdata[1]+"-"+sdata[0]);
                                Date c = Calendar.getInstance().getTime();
                                System.out.println("Current time => " + c);
                                ...
                                if(Integer.parseInt(cdata[2])== Integer.parseInt(sdata[0])) {
                                    ...
                                    if (Integer.parseInt(cdata[1]) == Integer.parseInt(sdata[1])) {
                                        if (Integer.parseInt(cdata[0]) == Integer.parseInt(sdata[2])) {
                                            ...
                                            // Toast.makeText(VehicelWise_Deatail.this, "gggggggg", Toast.LENGTH_SHORT).show();
                                            if (Integer.parseInt(cdata[0]) - 1 == Integer.parseInt(sdata[2])) {
                                                com_polling_status = "Valid";
                                                vtu_status.setText("Valid");
                                            }else {
                                                // Toast.makeText(VehicelWise_Deatail.this, "gggggggg", Toast.LENGTH_SHORT).show();
